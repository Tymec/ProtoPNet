...
export default function App() {
  ...
  const updateHabitatData = (bird: string) => {
    console.log(bird)
    setHabitatData(
      bird in habitats ? habitats[bird as keyof typeof habitats] : []
    );
  }
    ...
    <div className="min-h-screen bg-white p-8 dark:bg-slate-800">
      <div className="mx-auto flex w-3/4 flex-col gap-4">
        <div className="flex flex-row flex-wrap items-stretch justify-center gap-4">
          <div className={`flex-shrink flex-grow basis-1/7 ${loading ? 'animate-pulse' : ''}`}>
            <ImageDropzone
              onUpload={(file: File) => {
                setConfidenceData({});
                setHabitatData([]);
          ...
          <PredictionsCarousel confidenceData={confidenceData} loading={loading} onUpdateBird={updateHabitatData}/>
