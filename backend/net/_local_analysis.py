...
from helpers import find_high_activation_crop, makedir
...
if check_test_accu:
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=test_batch_size, shuffle=True, num_workers=4, pin_memory=False
    )
...
def save_prototype_self_activation(fname, epoch, index):
    p_img = plt.imread(
        os.path.join(load_img_dir, "epoch-" + str(epoch), "prototype-img-original_with_self_act" + str(index) + ".png")
    )
    # plt.axis('off')
    plt.imsave(fname, p_img)
...
def save_prototype_original_img_with_bbox(
    fname, epoch, index, bbox_height_start, bbox_height_end, bbox_width_start, bbox_width_end, color=(0, 255, 255)
):
    p_img_bgr = cv2.imread(
        os.path.join(load_img_dir, "epoch-" + str(epoch), "prototype-img-original" + str(index) + ".png")
    )
    cv2.rectangle(
        p_img_bgr, (bbox_width_start, bbox_height_start), (bbox_width_end - 1, bbox_height_end - 1), color, thickness=2
    )
...
def imsave_with_bbox(
    fname, img_rgb, bbox_height_start, bbox_height_end, bbox_width_start, bbox_width_end, color=(0, 255, 255)
):
...
for i in range(1, 11):
    ...
    save_prototype_original_img_with_bbox(
        fname=os.path.join(
            save_analysis_path, "most_activated_prototypes", "top-%d_activated_prototype_in_original_pimg.png" % i
        ),
    ...
    high_act_patch = original_img[
        high_act_patch_indices[0] : high_act_patch_indices[1], high_act_patch_indices[2] : high_act_patch_indices[3], :
    ]
    log("most highly activated patch of the chosen image by this prototype:")
    # plt.axis('off')
    plt.imsave(
        os.path.join(
            save_analysis_path, "most_activated_prototypes", "most_highly_activated_patch_by_top-%d_prototype.png" % i
        ),
        high_act_patch,
    )
    ...
    plt.imsave(
        os.path.join(
            save_analysis_path, "most_activated_prototypes", "prototype_activation_map_by_top-%d_prototype.png" % i
        ),
        overlayed_img,
    )
    log("--------------------------------------------------------------")
