    ...
    for v in cfg:
        if v == "M":
            rf_info = compute_layer_rf_info(
                layer_filter_size=2, layer_stride=2, layer_padding="SAME", previous_layer_rf_info=rf_info
            )
        else:
            rf_info = compute_layer_rf_info(
                layer_filter_size=3, layer_stride=1, layer_padding="SAME", previous_layer_rf_info=rf_info
            )

    proto_layer_rf_info = compute_layer_rf_info(
        layer_filter_size=prototype_kernel_size, layer_stride=1, layer_padding="VALID", previous_layer_rf_info=rf_info
    )
